...
const loadVoteIds = async (round) => (await fetchFirestoreCollection(VOTES_COLLECTION))
  .filter(({ data }) => Number(data.round) === Number(round))
  .map(({ data }) => data.voterId);

const getPublicState = async (req, res) => {
  const state = await getState();
  const session = sessionFromRequest(req);
  state.viewerVoteIds = state.phase === "voting" ? await loadVoteIds(state.currentRound) : [];
...
const submitVote = async (req, res) => {
  ...
  const voteId = `${state.currentRound}-${session.id}`;
  ...
  await createFirestoreDocument(VOTES_COLLECTION, voteId, { round: state.currentRound, voterId: session.id, allocations: cleaned, createdAt: now() });
...
const applyScoring = async (state, body) => {
  ...
  for (const player of participants) {
    const points = asInt(body?.scores?.[player.id] ?? 0, 0, 1000);
  ...
  if (state.currentRound < 4 && remaining < 4) throw Object.assign(new Error("Před finále musí zůstat alespoň 4 hráči."), { status: 400 });
  if (state.currentRound === 4 && remaining !== 4) throw Object.assign(new Error("Do pátého kola musí postoupit přesně 4 hráči."), { status: 400 });
...
const finalizeRoundTotals = async (state) => {
  ...
  if ([2, 4].includes(Number(state.currentRound))) {
    const votes = (await fetchFirestoreCollection(VOTES_COLLECTION)).filter(({ data }) => Number(data.round) === Number(state.currentRound));
...
const adminAction = async (req, res) => {
  ...
  if (action === "open_lobby") {
    if (!["not_started", "published"].includes(state.status)) throw Object.assign(new Error("Turnaj už byl otevřen."), { status: 409 });
    const players = await initializePlayers();
    state = defaultState(); state.status = "lobby"; state.phase = "lobby"; state.players = players;
...
const getAdminState = async (req, res) => {
  ...
  const state = await getState();
  const voteIds = state.phase === "voting" ? await loadVoteIds(state.currentRound) : [];
